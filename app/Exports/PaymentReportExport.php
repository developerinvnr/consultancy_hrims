...
class PaymentReportExport implements FromQuery, WithHeadings, WithMapping, WithStyles, ShouldAutoSize, WithTitle
{
	...
	public function query()
	{
		$query = SalaryProcessing::query()
			->with('processor') // Add this to load the processor relationship
	...
	protected function applyFilters($query)
	{
		...
		// ========== FINANCIAL YEAR & MONTH FILTER (Same as controller) ==========
		$month = $this->filters['month'] ?? null;
		$financialYear = $this->filters['financial_year'] ?? null;

		if ($month && $month != 'All' && $financialYear) {
	...
	public function title(): string
	{
		...
		if (!empty($this->filters['month']) && $this->filters['month'] != 'All') {
			$monthName = $this->getMonthName($this->filters['month']);
			$title .= '_' . $monthName;
		}
		if (!empty($this->filters['financial_year'])) {
			$title .= '_FY_' . $this->filters['financial_year'];
		}
		if (!empty($this->filters['status']) && $this->filters['status'] != 'All') {
			$title .= '_' . ucfirst($this->filters['status']);
		}
		return $title;
	}
