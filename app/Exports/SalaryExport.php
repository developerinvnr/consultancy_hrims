...
class SalaryExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithEvents
{
    ...
    {
        $salaryMonthStart = \Carbon\Carbon::create($this->year, $this->month, 1)
            ->startOfMonth()
            ->toDateString();

        $salaryMonthEnd = \Carbon\Carbon::create($this->year, $this->month, 1)
            ->endOfMonth()
            ->toDateString();
    ...
    {
        return [
            3 => [ // Header row (because you insert title rows)
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getColumnDimension('D')->setWidth(12); // Paid Days
                $event->sheet->getDelegate()->getColumnDimension('E')->setWidth(15); // Monthly
                $event->sheet->getDelegate()->getColumnDimension('F')->setWidth(15); // Extra
                $event->sheet->getDelegate()->getColumnDimension('G')->setWidth(15); // Deduction
                $event->sheet->getDelegate()->getColumnDimension('H')->setWidth(15); // Net Pay
                $event->sheet->getDelegate()->getColumnDimension('I')->setWidth(20); // Processed Date
                $event->sheet->getDelegate()->getColumnDimension('J')->setWidth(15); // Status
                ...
                $event->sheet->getStyle('E1:H' . $highestRow)
                    ->getAlignment()
                    ->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);

                // Format date column
                $event->sheet->getStyle('I2:I' . $highestRow)
                    ->getNumberFormat()
                ...
                $event->sheet->getStyle('A1:J' . ($highestRow + 2))
                    ->getBorders()
                    ->getAllBorders()
                    ->setBorderStyle(Border::BORDER_THIN);
            },
