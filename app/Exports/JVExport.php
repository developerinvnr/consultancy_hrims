    ...
    public function collection()
    {
        ...
        return $records->map(function ($rec) use ($narration) {
            ...
            return [
                ...
                'N/A', // Cost Center
                $rec->candidate->businessUnit->business_unit_code ?? 'N/A',
                'All Activity',
                $rec->candidate->work_location_hq ?? '',
                $rec->candidate->workState->state_code ?? '',
                'N/A',
                'All Crop',
                $rec->candidate->regionRef->focus_code ?? 'N/A',
                $rec->candidate->function->function_code ?? 'N/A',
                $rec->candidate->vertical->vertical_code ?? 'N/A',
                $rec->candidate->subDepartmentRef->focus_code ?? 'N/A',
                $rec->candidate->zoneRef->zone_code ?? 'N/A',
                'INDIRECT-MSC-17',
