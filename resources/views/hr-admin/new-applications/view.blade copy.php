                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <p class="mb-0 small">{{ $requisition->address_line_1 }}, {{ $requisition->city }},
                                            {{ $requisition->state_residence }} - {{ $requisition->pin_code }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card border h-100">
                                <div class="card-header bg-light py-2 d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <span class="badge bg-success ms-1">{{ count($agreementDocuments) }} Agreement(s)</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="card-body">
                                    ...
                                    <div class="mb-4">
                                        <h6 class="text-muted small mb-3">Application Documents</h6>
                                        ...
                                        <div class="text-center py-2">
                                            ...
                                            <p class="text-muted mt-1 small">No application documents uploaded</p>
                                        </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="mt-4 pt-3 border-top">
                                        <h6 class="text-muted small mb-3">Agreement Documents</h6>
                                        @foreach($agreementDocuments as $doc)
                                        <div class="document-item border rounded mb-2 p-2 {{ !$doc['has_file'] ? 'opacity-75' : '' }}">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="flex-grow-1">
                                                    <div class="d-flex align-items-center mb-1">
                                                        <span class="fw-semibold small {{ !$doc['has_file'] ? 'text-muted' : 'text-dark' }}">
                                                            {{ $doc['document_type'] }}
                                                        </span>
                                                        @if(!$doc['has_file'])
                                                        ...
                                                        @endif
                                                        @if($doc['type'])
                                                        <span class="badge bg-info text-white ms-1 small">{{ $doc['type'] }}</span>
                                                        @endif
                                                        @if(!empty($doc['agreement_number']))
                                                        <span class="badge bg-light text-dark ms-1 small">#{{ $doc['agreement_number'] }}</span>
                                                        @endif
                                                    </div>
                                                    <div class="small text-muted">
                                                        ...
                                                        <span class="ms-2 text-success">
                                                            <i class="ri-check-circle-line"></i> Available
                                                        </span>
                                                        @endif
                                                    </div>
                                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Application Document View Button Click
        ...
        viewDocumentBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                openDocumentModal(documentUrl, documentName);
            });
        });

        // Agreement Document View Button Click
        const viewAgreementDocumentBtns = document.querySelectorAll('.view-agreement-document-btn');
        viewAgreementDocumentBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                const documentUrl = this.getAttribute('data-document-url');
                const documentName = this.getAttribute('data-document-name');
                openDocumentModal(documentUrl, documentName);
