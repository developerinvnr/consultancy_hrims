$(document).ready(function () {
    ...
    $("#uploadSignedEmailForm").on("submit", function (e) {
        ...
        submitAgreementForm(
            $(this),
            "/hr-admin/agreement/{candidate}/upload-signed",
        );
    });
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (
                    ...
                    Swal.fire(
                        "Error!",
                        result.value?.message || "Something went wrong",
                        "error",
                    );
                    submitBtn
                        .prop("disabled", false)
    ...
    $("#endContractForm").on("submit", function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(
                    "/candidate/" + candidateId + "/deactivate",
                    formData,
                    function (response) {
                        Swal.fire(
                            "Success",
            ...
            success: function (response) {
                ...
                if (data.current) {
                    ...
                    $("#reporting_manager_id").val(
                        data.current.reporting_manager_employee_id,
                    );
                }
...
================================ */
    ...
    $(document).on("click", ".create-file-btn", function () {
        ...
        Swal.fire({
            ...
            html: `
            <p class="text-muted">
            Please confirm that all documents are verified:
            </p>

            <div style="text-align:left">
                <label><input type="checkbox" id="chkCourier"> Courier received</label><br>
                <label><input type="checkbox" id="chkAgreement"> Signed agreement verified</label><br>
                <label><input type="checkbox" id="chkDocs"> All documents verified</label>
            </div>
        `,
            icon: "question",
            showCancelButton: true,
            ...
            preConfirm: () => {
                if (
                    !$("#chkCourier").is(":checked") ||
                    !$("#chkAgreement").is(":checked") ||
                    !$("#chkDocs").is(":checked")
                ) {
